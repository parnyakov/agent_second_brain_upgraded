...
_STATUS_MESSAGES = {
    ...
    "logged_out": (
        "🔑 <b>Нужен вход в подписку ИИ.</b> На сервере выполните "
        "<code>dbrain login</code>, затем /process."
    ),
    "timeout": "⌛ <b>Превышено время ожидания ответа.</b> Попробуйте ещё раз.",
    "error": "❌ <b>Ошибка сессии.</b> Попробуйте позже.",
    "reset": "⏹ <b>Прервано командой /reset.</b> Запустите ещё раз.",
}
...
class ClaudeProcessor:
    ...
    def process_daily(self, day: date | None = None) -> dict[str, Any]:
        ...
        if not skill_content.strip():
            logger.error("dbrain-processor skill is missing or empty")
            return {
                "error": (
                    "Не найден скилл ежедневной обработки "
