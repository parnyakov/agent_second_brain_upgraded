...
def _daily(tmp_path):
    ...
    skill = tmp_path / ".claude" / "skills" / "dbrain-processor" / "SKILL.md"
    skill.parent.mkdir(parents=True, exist_ok=True)
    skill.write_text("# dbrain-processor\n")
    return date(2026, 6, 7)


def test_process_daily_without_skill_is_a_clear_error(tmp_path):
    """No silent run with empty instructions: a missing /process skill is
    reported, and the session is never asked."""
    (tmp_path / "daily").mkdir()
    (tmp_path / "daily" / "2026-06-07.md").write_text("# d\n")
    sess = FakeSession(AskResult("ok", reply="x"))
    r = ClaudeProcessor(tmp_path, session=sess).process_daily(date(2026, 6, 7))
    assert "dbrain-processor" in r["error"]
    assert sess.prompts == []
