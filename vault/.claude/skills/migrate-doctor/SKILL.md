...
3. **Prefer re-running `bash upgrade.sh`** after each fix — it is idempotent. Fix the obstacle, not the symptom.
   Exit code 3 is not a broken migration: everything was installed and only the
   final health check is red (`dbrain doctor` shows the ❌ line, usually a missing
   engine login → `dbrain login`).
